        ...
        <div className="peek-shell space-y-6">
          <div className="peek-header peek-header--admin flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-gray-500">لوحة تشغيل يومية • مرحباً {user?.display_name}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="peek-kpi"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-gray-500 text-sm">الجلسات النشطة</p><p className="peek-kpi-value mt-1">{activeSessions.length}</p></div><Activity className="w-10 h-10 text-blue-500" /></div></CardContent></Card>
            <Card className="peek-kpi"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-gray-500 text-sm">إيرادات اليوم</p><p className="peek-kpi-value mt-1">{stats?.revenue?.total || 0} د.أ</p></div><DollarSign className="w-10 h-10 text-emerald-500" /></div></CardContent></Card>
            <Card className="peek-kpi"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-gray-500 text-sm">جلسات اليوم</p><p className="peek-kpi-value mt-1">{stats?.sessions?.total || 0}</p></div><Clock className="w-10 h-10 text-violet-500" /></div></CardContent></Card>
            <Card className="peek-kpi"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-gray-500 text-sm">رسوم الوقت الإضافي</p><p className="peek-kpi-value mt-1">{stats?.revenue?.overtime || 0} د.أ</p></div><TrendingUp className="w-10 h-10 text-orange-500" /></div></CardContent></Card>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="peek-card"><CardHeader><CardTitle className="text-lg">تفاصيل الإيرادات</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex justify-between"><span className="text-gray-600">دخول بالساعة</span><span className="font-medium">{stats?.revenue?.walk_in || 0} د.أ</span></div><div className="flex justify-between"><span className="text-gray-600">الاشتراكات</span><span className="font-medium">{stats?.revenue?.subscriptions || 0} د.أ</span></div><div className="flex justify-between"><span className="text-gray-600">باقات الزيارات</span><span className="font-medium">{stats?.revenue?.visit_packs || 0} د.أ</span></div><div className="flex justify-between"><span className="text-gray-600">الوقت الإضافي</span><span className="font-medium">{stats?.revenue?.overtime || 0} د.أ</span></div><div className="border-t pt-3 flex justify-between font-bold"><span>الإجمالي</span><span className="text-green-600">{stats?.revenue?.total || 0} د.أ</span></div></CardContent></Card>
            <Card className="peek-card"><CardHeader><CardTitle className="text-lg">المبيعات اليوم</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex justify-between"><span className="text-gray-600">اشتراك شهري شامل</span><span className="font-medium">{stats?.sales?.subscriptions?.by_plan?.MONTHLY_ALL_ACCESS || 0}</span></div><div className="flex justify-between"><span className="text-gray-600">نصف يوم صباحي</span><span className="font-medium">{stats?.sales?.subscriptions?.by_plan?.HALF_DAY_MORNING || 0}</span></div><div className="flex justify-between"><span className="text-gray-600">نصف يوم مسائي</span><span className="font-medium">{stats?.sales?.subscriptions?.by_plan?.HALF_DAY_EVENING || 0}</span></div><div className="flex justify-between"><span className="text-gray-600">باقات زيارات</span><span className="font-medium">{stats?.sales?.visit_packs || 0}</span></div></CardContent></Card>
          </div>
        </div>
      </div>
    );
