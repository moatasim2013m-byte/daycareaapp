...
describe('ParentDashboard', () => {
  ...
  it('renders parent dashboard sections', async () => {
    ...
    api.get.mockImplementation((url) => {
      ...
      if (url === '/parent/payments') return Promise.resolve({ data: { subscription_status: 'ACTIVE', payment_history: [{ payment_id: 'p1', description: 'Monthly', amount: 100, currency: 'JOD', status: 'COMPLETED' }] } });
      if (url === '/parent/messages') return Promise.resolve({ data: [{ id: 'm1', subject: 'Hello', body: 'Message' }] });
      if (url === '/parent/bookings') return Promise.resolve({ data: { session_visits: [{ booking_id: 'b1', service: 'Session', date: '2026-03-10', time: '10:00', status: 'CONFIRMED' }] } });
